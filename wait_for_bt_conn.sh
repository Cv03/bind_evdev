...
watch_expression="type='signal',sender='org.bluez',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged'"
dbus-monitor --system "$watch_expression" 2> /dev/null | \
while read -r line; do
    ...
    if [[ "$line" == *"string \"Connected\""* ]]; then
        ...
        if [[ "$val_line" == *"boolean true"* ]]; then
            if [[ -n $name ]]; then
                device_name="$(busctl --system get-property org.bluez \
                    "$device_path" org.bluez.Device1 Name)"
                if [[ ! $device_name =~ $name_pattern ]]; then
                    continue
                fi
            fi
            if [[ -n $address ]]; then
                device_address="$(busctl --system get-property org.bluez \
                    "$device_path" org.bluez.Device1 Address)"
