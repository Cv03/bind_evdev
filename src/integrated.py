# pyright: reportUnusedCallResult=false, reportOptionalCall=false
...
def set_kpenter_mode(to: str):
    def fn(bind: Bind, _e: InputEvent):
...
def unset_kpenter_mode(bind: Bind, _e: InputEvent):
    mode: str | None = bind.data.get('kpenter_mode')
    if mode == 'alt_tab':
...
bind([k.KEY_LEFTSHIFT, k.KEY_TAB], to_key=k.KEY_KP4, on='tap', before=[set_kpenter_mode('alt_tab')])
bind(k.KEY_TAB, to_key=k.KEY_KP5, on='tap', before=[set_kpenter_mode('alt_tab')])

# Minimize window
bind([k.KEY_LEFTMETA, k.KEY_PAGEDOWN], to_key=[k.KEY_KPENTER, k.KEY_KP1], on='tap', before=[set_kpenter_mode('window')])
bind(k.KEY_ESC, to_key=k.KEY_KP1)
# Maximize window
bind([k.KEY_LEFTMETA, k.KEY_PAGEUP], to_key=[k.KEY_KPENTER, k.KEY_KP2], on='tap', before=[set_kpenter_mode('window')])
# Zoom out
bind([k.KEY_LEFTMETA, k.KEY_MINUS], to_key=k.KEY_KP2, on='tap')
# Close window
bind([k.KEY_LEFTALT, k.KEY_F4], to_key=[k.KEY_KPENTER, k.KEY_KP3], on='tap', before=[set_kpenter_mode('window')])
# Zoom in
bind([k.KEY_LEFTMETA, k.KEY_EQUAL], to_key=k.KEY_KP3, on='tap')
bind(None, to_key=k.KEY_KPENTER, before=[unset_kpenter_mode])
